...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        start = 0
        while place_tracker.name != 'Hive' and start <= self.max_range:
            if self.min_range <= start and len(place_tracker.bees) > 0:
                return rANTdom_else_none(place_tracker.bees)
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.is_True_Queen = True if QueenAnt.count == 0 else False
        QueenAnt.count += 1
        # END Problem 13
        ...
        if not self.is_True_Queen:
            ...
            while curr is not None:
                if curr.ant is not None:
                    if isinstance(curr.ant, ContainerAnt) and curr.ant.contained_ant is not None and not curr.ant.contained_ant.damage_doubled:
                        curr.ant.contained_ant.damage *= 2
                        curr.ant.contained_ant.damage_doubled = True
                    if not curr.ant.damage_doubled:
                        curr.ant.damage *= 2
                        curr.ant.damage_doubled = True
                curr = curr.exit
        # END Problem 13
...
def apply_status(status, bee, length):
    ...
    if len(bee.actions) == 0:
        bee.actions.append(original_action)
    status_action, type = status(bee.actions[0], bee)
    if bee.status[type] == 0:
        bee.status[type] = length
    else:
        bee.status[type] += length
