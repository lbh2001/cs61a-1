...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0,score1):
        ...
        announcement = lambda player_score: str(player_score)+ " point(s)! That's the biggest gain yet for Player "+ str(who)
        if score - last_score > running_high:
            print(announcement(score-last_score))
...
def swap_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if (free_bacon(opponent_score) >= cutoff and not is_swap(score, opponent_score)) \
    or (is_swap(score, opponent_score) and score < opponent_score):
        return 0
    return num_rolls
    # END PROBLEM 11
