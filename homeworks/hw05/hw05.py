...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    errors = []
    def withdraw(amount, atmpt_pass):
        nonlocal balance
        if len(errors) < 3:
            if atmpt_pass == password:
                if balance > amount:
                    balance -= amount
                    return balance
                else:
                ...
                errors.append(atmpt_pass)
                return 'Incorrect password'
        else:
            return "Too many incorrect attempts. Attempts: "+ str(errors)
    return withdraw
