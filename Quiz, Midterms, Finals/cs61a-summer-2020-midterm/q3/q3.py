...
def drawing(piano, t):
    ...
    def graft(t, bs):
        ...
        """
        if is_leaf(bs):
            return piano(label(bs))
        new_branches = [tree(label(sub),branches=[graft(piano(label(b)),[sub for sub in b]) for b in bs]) for sub in branches(t)]
        return tree(label(t), branches=new_branches)
    base_t = piano(label(t))
    bs = [b for b in branches(t)]
